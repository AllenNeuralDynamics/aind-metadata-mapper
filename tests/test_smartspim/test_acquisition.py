...
from tests.test_smartspim.example_metadata import (
    ...
    example_session_end_time, example_imaging_info_from_slims,
)
...
class TestSmartspimETL(unittest.TestCase):
    ...
    @patch("aind_metadata_mapper.smartspim.acquisition.SmartspimETL._extract_metadata_from_microscope_files")
    def test_extract_metadata_from_microscope_files(self, mock_extract: MagicMock):
        ...
        result = self.example_smartspim_etl_success._extract_metadata_from_microscope_files()
    ...
    def test_extract_metadata_from_slims_success(self, mock_get: MagicMock):
        """Tests that metadata is extracted using the metadata-service as expected."""
        ...
        result = self.example_smartspim_etl_success._extract_metadata_from_slims(
    ...
    def test_extract_metadata_from_slims_no_data_found(self, mock_get):
        """_extract_metadata_from_slims returns no data found."""
        mock_response = MagicMock()
    ...
    def test_extract_metadata_from_slims_multiple_responses(self, mock_get):
        """_extract_metadata_from_slims raises ValueError if multiple sessions found"""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"data": [{"example_id": 1}, {"example_id": 2}]}
        mock_response.raise_for_status = MagicMock()
        mock_get.return_value = mock_response
    ...
    def test_map_axes(self):
        """_map_axes correctly maps anatomical directions to ImageAxis objects"""
        x_dir = "Left to Right"
    ...
    def test_map_immersion_medium(self):
        """_map_immersion_medium maps medium strings to ImmersionMedium enum correctly"""
        mm = SmartspimETL._map_immersion_medium
        self.assertEqual(SlimsImmersionMedium.DIH2O.value, "diH2O")
        self.assertEqual(SlimsImmersionMedium.CARGILLE_OIL_152.value, "Cargille Oil 1.5200")
        self.assertEqual(ImmersionMedium.WATER, mm(SlimsImmersionMedium.DIH2O.value))
        self.assertEqual(ImmersionMedium.OIL, mm(SlimsImmersionMedium.CARGILLE_OIL_152.value))
        self.assertEqual(ImmersionMedium.OIL, mm(SlimsImmersionMedium.CARGILLE_OIL_153.value))
        self.assertEqual(ImmersionMedium.ECI, mm(SlimsImmersionMedium.ETHYL_CINNAMATE.value))
    ...
    def test_map_processing_steps(self):
        ...
        steps = self.example_smartspim_etl_success._map_processing_steps(slims_data)
        ...
        for expected in expected_steps:
            actual = next(s for s in steps if s.channel_name == expected.channel_name)
            self.assertCountEqual(
                actual.process_name,
                expected.process_name,
                msg=f"Mismatch in process_name for channel {expected.channel_name}"
            )

    def test_transform(self):
        """Tests that _transform combines microscope metadata and SLIMS data into a complete Acquisition model"""
        metadata_dict = {
            "session_config": example_metadata_info["session_config"],
            "wavelength_config": example_metadata_info["wavelength_config"],
            "tile_config": example_metadata_info["tile_config"],
        ...
        acq = self.example_smartspim_etl_success._transform(metadata_dict=metadata_dict, slims_data=slims_data)
    ...
    def test_transform_fail_mouseid(self, mock_extract: MagicMock):
        ...
        test_extracted = self.example_smartspim_etl_fail_mouseid._extract_metadata_from_microscope_files()
    ...
    def test_transform_missing_axes(self):
        """_transform throws an error if any axis direction is missing in SLIMS data"""
        # SLIMS data with missing z_direction (set to None)
        slims_data = copy.deepcopy(example_imaging_info_from_slims)
        ...
        # If an axis direction is None, get_anatomical_direction will fail (AttributeError)
        with self.assertRaises(Exception):
            self.example_smartspim_etl_success._transform(metadata_dict=metadata_dict, slims_data=slims_data)

    @patch("aind_data_schema.base.AindCoreModel.write_standard_file")
    @patch("aind_metadata_mapper.smartspim.acquisition.SmartspimETL._extract_metadata_from_slims")
    @patch("aind_metadata_mapper.smartspim.acquisition.SmartspimETL._extract_metadata_from_microscope_files")
    def test_run_job_success(self, mock_extract_microscope, mock_extract_slims, mock_write):
        """run_job integrates extraction, transform, and load to produce a JobResponse"""
        mock_extract_microscope.return_value = {
            ...
            "session_end_time": example_session_end_time,
            "filter_mapping": example_filter_mapping,
            "session_start_time": datetime.strptime("2024-10-10_10-10-10", "%Y-%m-%d_%H-%M-%S")
        }
