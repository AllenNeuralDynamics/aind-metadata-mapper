...
class CamstimEphysSessionEtl(
    ...
    def clean_isi_discrepancies(self, sync_start_times, sync_end_times, pkl_isis, mismatch_threshold=0.015):
        """
        When there are extra blips in the sync, the sync times and pkl ISIs
        can get out of alignment. This function removes sync times and related pkl times
        that are likely erroneous based on large discrepancies between expected ISIs
        from the pkl and observed ISIs from the sync.
        """
        assert len(sync_start_times) == len(sync_end_times), "Sync start and end times must be the same length"
        ...
        approx_pkl_time = 0
        while s_end < len(sync_end_times)-1 and s_start < len(sync_start_times) and p < len(pkl_isis):
            assert s_start > s_end, "Sync start index must be after sync end index to get a valid ISI"
            approx_pkl_time = sync_end_times[s_end] + pkl_isis[p]
            # each ISI is the time between the current end idx and next start idx
            ...
            # if expected and observed ISI are similar, keep this start time and iterate
        ...
        # always keep trailing pkl interval (it is not followed by a sync pulse at end of session)
    ...
    def build_optogenetics_table(self):
        ...
        assert len(start_times) == len(stop_times), "Number of opto start times does not match number of stop times"
        start_times, keep_pkl_idxs = self.clean_isi_discrepancies(
            start_times,
            stop_times,
            opto_file["opto_ISIs"],
        )
        condition_nums = [str(item) for item in opto_file["opto_conditions"][keep_pkl_idxs]]
