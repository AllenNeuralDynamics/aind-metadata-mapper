...
from aind_data_schema.core.procedures import SpecimenProcedure, SpecimenProcedureType, Reagent, Surgery, Procedures
...
class JobSettings(BaseSettings):
    ...
    allow_validation_errors: bool = Field(
        False,
        description="Whether or not to allow validation errors."
    )
...
class U19Etl(GenericEtl[JobSettings]):
    ...
    def find_sheet_row(self, subj_id):
        ...
        for sheet in self.tissue_sheets:
            if int(subj_id) in sheet['SubjInfo']['Unnamed: 0_level_1']['Mouse ID'].tolist():
                return sheet.loc[sheet['SubjInfo']['Unnamed: 0_level_1']['Mouse ID'] == int(subj_id)]
    ...
    def load_specimen_procedure_file(self):
        ...
        for sheet_name in self.job_settings.tissue_sheet_names:
            df = pd.read_excel(self.job_settings.tissue_sheet_path, sheet_name=sheet_name, header=[0,1,2])
            self.tissue_sheets.append(df)    

    def extract_spec_procedures(self, subj_id, row):

        default_protocol = "dx.doi.org/10.17504/protocols.io.bgpujvnw"
        default_source = Organization.LIFECANVAS

        subj_id = str(row['SubjInfo']['Unnamed: 0_level_1']['Mouse ID'].iloc[0]).strip().lower()
        clearing_method = row['SubjInfo']['Unnamed: 1_level_1']['ClearingMethod'].iloc[0]
        ...
        shield_on_reagent = Reagent(
            name="SHIELD ON",
            source=default_source,
            lot_number=shield_on_lot
        )

        passive_delipidation_dates = row['Passive delipidation']['24 Hr Delipidation ']['Date(s)'].iloc[0]
        if not pd.isna(passive_delipidation_dates):
            passive_delipidation_dates = strings_to_dates(get_dates(passive_delipidation_dates))
        passive_conduction_buffer_lot = row['Passive delipidation']['Delipidation Buffer']['Lot#'].iloc[0]
        if pd.isna(passive_conduction_buffer_lot):
            passive_conduction_buffer_lot = "unknown"
        passive_delip_notes = row['Passive delipidation']['Notes']['Unnamed: 12_level_2'].iloc[0]           
        ...
        active_delipidation_dates = row['Active Delipidation']['Active Delipidation']['Date(s)'].iloc[0]
        if not pd.isna(active_delipidation_dates):
            active_delipidation_dates = strings_to_dates(get_dates(active_delipidation_dates))
        active_conduction_buffer_lot = row['Active Delipidation']['Conduction Buffer']['Lot#'].iloc[0]
        if pd.isna(active_conduction_buffer_lot):
            active_conduction_buffer_lot = "unknown"
        active_delip_buffer = row['Active Delipidation']['Active Delipidation']['Date(s)'].iloc[0]
        active_delip_settings = row['Active Delipidation']['Settings']['Unnamed: 16_level_2'].iloc[0]
        active_delip_notes = row['Active Delipidation']['Notes']['Unnamed: 17_level_2'].iloc[0]
