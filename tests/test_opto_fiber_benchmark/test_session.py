...
from aind_metadata_mapper.opto_fiber_benchmark.session import OptoFiberBenchmark
...
class TestOptoFiberBenchmark(unittest.TestCase):
    ...
    def setUp(self):
        ...
        pd.DataFrame({"SoftwareTS": [0, 1, 2]}).to_csv(signal_file, index=False)
    ...
    def test_extract_session_start_time(self):
        ...
        data_files = list(Path(self.js.fiber.data_directory).glob("*Signal*.csv"))
        start_time = etl._extract_session_start_time(data_files)
        expected_time = datetime.strptime("Signal_2025-10-07T12_00_00", "Signal_%Y-%m-%dT%H_%M_%S")
        self.assertEqual(start_time, expected_time)

    def test_extract_stimulus_epochs(self):
        etl = OptoFiberBenchmark(self.js)
        data_files = list(Path(self.js.fiber.data_directory).glob("*Stim*.csv"))
        stim_info = etl._extract_stimulus_epochs([Path("/dummy/Signal_2025-10-07T12_00_00.csv")] + data_files)
    ...
    def test_run_job_creates_file(self):
        etl = OptoFiberBenchmark(self.js)
        with patch("aind_data_schema.core.session.Session.write_standard_file") as mock_write:
